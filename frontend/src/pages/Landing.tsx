...
const ROTATING_WORDS = ["courses", "workshops", "skills", "networks", "careers"];
    ...
    <>
      ...
      <section className="hero hero-animated">
        ...
        <div className="container" style={{ position: "relative", zIndex: 2 }}>
          ...
          <div className="row" style={{ justifyContent: "center", marginTop: "1.5rem" }}>
            ...
            <Link to="/courses" className="btn btn-secondary btn-hero">Browse Courses</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "3rem 0" }}>
        <div className="container">
          ...
          <div className="grid grid-3">
            {[
              { label: "AI", title: "AI Recommendations", desc: "Semantic matching finds courses that truly fit your goals, not just keyword matches." },
              ...
              { label: "SF", title: "Substitute Finder", desc: "Course unavailable? AI finds alternatives covering the same skills." },
              { label: "ML", title: "Multilingual Support", desc: "English, Mandarin, Malay, and Tamil -- listings, search, and AI chatbot." },
              { label: "AL", title: "AI Auto Listing", desc: "Organisers paste a description, AI generates a complete course listing." },
            ].map((f, i) => (
      ...
      <section style={{ padding: "3rem 0", background: "var(--bg-subtle)" }}>
        <div className="container" style={{ textAlign: "center" }}>
          ...
          <p>From AI fundamentals to cloud computing, find what sparks your interest.</p>
          <div className="row" style={{ justifyContent: "center", gap: "0.6rem", marginTop: "1.5rem" }}>
            {["AI", "Software Engineering", "Cybersecurity", "Cloud Computing", "Data Science", "Digital Marketing", "Design", "Leadership", "Project Management", "Finance", "Entrepreneurship", "Career Development"].map((c, i) => (
              <Link key={c} to={`/courses?category=${encodeURIComponent(c)}`} className="badge category-badge" style={{ padding: "0.4rem 0.8rem", fontSize: "0.85rem", animationDelay: `${i * 0.05}s` }}>{c}</Link>
            ))}
          </div>
        </div>
      </section>
